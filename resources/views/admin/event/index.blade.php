...
@section('script')
    <script>
        ...
        $(document).on('click', '.button-dokumen', function(e) {
            e.preventDefault();
            let label = $(this).attr('data-label');
            ...
            $('#kt_table_data').DataTable({
                ...
                columns: [{
                    ...
                    render: function(data) {
                        if (!data) return "-"; // Jika data kosong

                        moment.locale('id'); // Set locale ke Indonesia

                        // Pisahkan tanggal berdasarkan " to "
                        let dates = data.split(' to ');
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        result =
                            `
                                <!--begin::Overlay-->
                                <a class="d-block overlay fancybox" data-fancybox="lightbox-group" href="{{ asset('/public/event-banner/${data}') }}">
                                    <!--begin::Image-->
                                    <div class="overlay-wrapper bgi-no-repeat bgi-position-center bgi-size-cover card-rounded min-h-175px"
                                        style="background-image:url('/public/event-banner/${data}')">
                                    </div>
                                    <!--end::Image-->
                    ...
                    render: function(data, type, full, meta) {
                        return `
                            <div class="d-flex gap-2" style="flex-flow: wrap;">
                                ...
                                <a href="javascript:;" type="button" data-uuid="${data}" data-label="Event" class="btn btn-danger button-delete btn-icon btn-sm">
                                    <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <a href="javascript:;" type="button" data-uuid="${data}" data-label="${full.status_dukumen == false ? 'Buka' : 'Tutup'}" class="btn btn-success button-dokumen btn-sm">
                                    ${full.status_dukumen == false ? 'Buka Dokumen' : 'Tutup Dokumen'}
                                </a>
                                ...
                                <a href="javascript:;" data-uuid="${data}" class="btn btn-secondary btn-sm button-qrcode">
                                    Cetak QrCode
                                </a>

                                <a href="javascript:;" data-uuid="${data}" class="btn btn-dark btn-sm  button-list-absen">
                                    List Absensi
                                </a>
                            </div>
                        `;
