...
@section('script')
    <script>
        ...
        $(document).on('click', '#button-side-form', function() {
            window.location.href = "{{ url('admin/add-daftarmakassar') }}";
        })

        $(document).on('click', '.button-update', function(e) {
            e.preventDefault();
            window.location.href = "{{ url('admin/edit-daftarmakassar/') }}" + $(this).attr('data-uuid');
        })

        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = "{{ url('admin/delete-daftarmakassar/') }}" + $(this).attr('data-uuid');
        ...
        const initDatatable = async () => {
            ...
            $('#kt_table_data').DataTable({
                ...
                processing: true,
                ajax: "{{ url('admin/get-daftarmakassar') }}",
                columns: [{
                    data: null,
                    ...
                    render: function(data, type, row) {
                        let formattedNilai = new Intl.NumberFormat('id-ID', {
                            useGrouping: true
                        }).format(row.nilai);

                        return formattedNilai + ' ' + row.satuan;
                    }
                }, {
                    data: 'position',
                    className: 'text-center',
                    ...
                    render: function(data, type, full, meta) {
                        return `
                            <div class="d-flex gap-2" style="flex-flow: wrap;">
                                ...
                                <a href="javascript:;" type="button" data-uuid="${data}" data-label="Daftar makassar dalam angka" class="btn btn-danger button-delete btn-icon btn-sm">
                                    <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <a href="javascript:;" type="button" data-uuid="${data}" class="btn btn-success button-up btn-sm">
                                    Naikkan
                                </a>

                                <a href="javascript:;" type="button" data-uuid="${data}" class="btn btn-primary button-down btn-sm">
                                    Turunkan
                                </a>
                            </div>
                        `;
