                    ...
                    <div class="card-body p-0">
                        ...
                        <div class="card-body hover-scroll-overlay-y">
                            <form class="form-data" enctype="multipart/form-data">
                                ...
                                @foreach ($data->indikator ?? [] as $key => $val)
...
@section('script')
    <script>
        ...
        const generateSchoolYears = (startYear, selectedYear) => {
            ...
            for (let year = startYear; year <= currentYear; year++) {
                years.push({
                    text: year,
                    value: year,
                    selected: parseInt(selectedYear) === year
                });
            }

            // Balik urutan tahun agar tahun terbaru berada di atas
            years.reverse();

            return years;
        };
        // Pastikan nilai tahun terisi dari Blade ke JS dengan benar
        const selectedYear = {{ json_encode((int) $data->tahun) }};
        const dataYears = generateSchoolYears(2000, selectedYear);
        ...
        $(document).on('submit', ".form-data", function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ url('admin/update-ipp/') }}" + lastPart,
                ...
                success: function(response) {
                    ...
                    if (response.success == true) {
                        swal
                            .fire({
                                text: `IPP berhasil di Update`,
                                icon: "success",
                                showConfirmButton: false,
                                timer: 1500,
                            })
                            .then(function() {
                                window.location.href = "{{ url('admin/ipp') }}";
                            });
                    } else {
                        $("form")[0].reset();
